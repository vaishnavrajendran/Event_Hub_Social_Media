...
export default function EnhancedTable() {
  ...
  const info = allUsers
    .filter((item) => item.isReported.length !== 0)
    .map((item) => ({
      id: item._id,
      UserName: item.firstName,
      Total_Reports: item.isReported.length,
  ...
  const blockUser = async (id) => {
    try {
      const response = await fetch(
        `http://localhost:3001/admin/${id}/blockUser`,
        {
          method: "PATCH",
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="right">{row.Total_Reports}</TableCell>
